...
class DB
{
    ...
    public function conn()
    {
        try {
            ...
            //Host configuration
            $dsn = "mysql:host=localhost;dbname=jb;charset=utf8mb4";
            //Create PDO
...
class Gebruikers extends DB
{
    ...
    public function create($voornaam, $tussenvoegsel, $achternaam, $email, $wachtwoord, $wachtwoord2)
    {
        ...
        try {
            ...
            $this->conn();
            // Define SQL Query
            $sql = "INSERT INTO `medewerker` (voornaam, tussenvoegsels, achternaam, email, wachtwoord) VALUES (:voornaam, :tussenvoegsels, :achternaam, :email, :wachtwoord)";
            // Prepare SQL
    ...
    public function update($voornaam, $tussenvoegsels, $achternaam, $email, $oudwachtwoord, $wachtwoord)
    {
        ...
        try {
            ...
            $sql2 = "SELECT wachtwoord FROM medewerker WHERE id_gebruiker = :id";
            //Prepare SQL
    ...
    public function delete($user_id, $email)
    {
        ...
        $this->conn();
        // Define SQL Query
        $sql = "DELETE FROM `medewerker` WHERE id_gebruiker = :userid AND email = :email";
        // Prepare SQL
        $stmt = $this->conn->prepare($sql);
...
public function setJongeren($voornaam, $tussenvoegsels, $achternaam, $roepnaam, $huisnummer, $straat, $woonplaats, $postcode, $geboortedatum, $inschrijfdatum, $nationaliteit, $opleiding, $droombaan, $baan, $activiteitID)
{
    // Create a connection with the database
    $this->conn();
    // Define SQL Query
    $sql = "INSERT INTO `ingeschrevenjongeren` (voornaam, tussenvoegsels, achternaam, roepnaam, huisnummer, straat, woonplaats, postcode, geboortedatum, inschrijfdatum, nationaliteit, opleiding, droombaan, baan, activiteitID) VALUES (:voornaam, :tussenvoegsels, :achternaam, :roepnaam, :huisnummer, :straat, :woonplaats, :postcode, :geboortedatum, :inschrijfdatum, :nationaliteit, :opleiding, :droombaan, :baan, :activiteitID)";
    // Prepare SQL
    $stmt = $this->conn->prepare($sql);
    // Bind value with the named placeholder	
    $stmt->bindParam(':voornaam', $voornaam);
    ...
    $stmt->bindParam(':droombaan', $droombaan);
    $stmt->bindParam(':baan', $baan);
